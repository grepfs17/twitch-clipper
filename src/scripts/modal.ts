...
function initQualitySelector() {
    if (!elements.qualitySelectTrigger || !elements.qualitySelect || !elements.qualitySelectOptions) return;
...
async function downloadClip(quality: string) {
    ...
    try {
        const response = await fetch(`/api/clips/download?url=${encodeURIComponent(currentClipUrl)}&quality=${encodeURIComponent(quality)}`);
        ...
        if (total > 0 && response.body) {
            ...
            const filename = filenameMatch ? filenameMatch[1] : `${slugFromUrl(currentClipUrl)}.mp4`;
