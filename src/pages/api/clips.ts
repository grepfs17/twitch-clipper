...
import { getAccessToken, getBroadcasterId, getClips, getGames } from '../../lib/twitch';
...
export const GET: APIRoute = async ({ request }) => {
    ...
    try {
        ...
        const gameIds = [...new Set(clipsData.data.map((clip: any) => clip.game_id))].filter(id => id !== '') as string[];
        ...
        const clips = clipsData.data.map((clip: any) => ({
            ...clip,
            game_name: gameMap[clip.game_id] || (clip.game_id ? 'Loading...' : 'No Category')
        }));
